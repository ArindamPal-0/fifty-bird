...
function PlayState:update(dt)
    ...
    -- we need this second loop, rather than deleting in the previous loop, because modifying the table in-place without explicit keys will result in skipping the next pipe, since all implecit keys (numerical indices) are automatically shifted down after a table removal
    for k, pair in pairs(self.pipePairs) do
        if pair.remove then
